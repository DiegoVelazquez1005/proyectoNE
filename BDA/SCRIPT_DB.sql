...
FROM dba_tables
WHERE table_name = 'USUARIO'; --USERS

SELECT table_name, tablespace_name
FROM dba_tables
WHERE table_name = 'CATEGORIA'; --SYSTEM

SELECT table_name, tablespace_name
FROM dba_tables
WHERE table_name = 'NOTA'; --SYSTEM

SELECT table_name, tablespace_name
FROM dba_tables
WHERE table_name = 'COMPARTE'; --SYSTEM
...
DATAFILE 'C:\app\sergi\product\21c\oradata\XE\USER_DATA.DBF' SIZE 6G
AUTOEXTEND ON
NEXT 600K;
...
CREATE ROLE NK_MANAGER_ROLE; --Se asignara al usuario DB_admin
...
CREATE ROLE NK_DEVELOPER_ROLE; -- Asignara al app_user
...
CREATE ROLE NK_REPORTING_ROLE; --Asignara al app_user
...
CREATE ROLE NK_DATA_ENTRY_ROLE;-- Asignar al app_user
